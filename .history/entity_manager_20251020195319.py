...
class Game:
    ...
    def level_up_menu(self,player):
        while True:
            ...
            level_choice = self.get_choices("What will you level up?", [level_options["1"]["variable"],level_options["2"]["variable"]])
            if level_choice=="0":
                self.live(player)
            elif level_choice:
                current_level =level_choice
                print(f"Current {level_options}: {current_level}")
                level_amount = input(f"By how much do you want to raise your {level_options} (enter 0 to go back): ")
